...
def extract_category_objective(
    ...
    Returns:
        1. pandas.DataFrame:
            Flattened Store Metadata DataFrame
    """
    ...
    try:
        ...
        print(
            ...
            f"{scopes} for Store Metadata extraction."
        )
    ...
    # Make Gspread API call for Store Metadata
    try:
        
        print(
            "🔍 [EXTRACT] Extracting Store Metadata in worksheet_name "
        ...
        if not values:
            ...
            print(
                "⚠️ [EXTRACT] Completely extracted Store Metadata from worksheet_name "
        ...
        print(
            "✅ [EXTRACT] Successfully extracted Store Metadata from worksheet_name "
    ...
    # Worksheet not found
        ...
        error = RuntimeError(
            "❌ [EXTRACT] Failed to extract Store Metadatae due to worksheet "
    ...
    # Unauthorized credentials
        ...
        error = RuntimeError(
            "❌ [EXTRACT] Failed to extract Store Metadata due to unauthorized Google credentials then manual re-authentication is required."
        )
    ...
    # Retryable API error
        if status in {
            ...
            error = RuntimeError(
                "⚠️ [EXTRACT] Failed to extract Store Metadata for worksheet_name "
    ...
    # Non-retryable access error
        if status in {
            ...
            error = RuntimeError(               
                "❌ [EXTRACT] Failed to extract Store Metadata for worksheet_name "
